...
# Bước 2: Huấn luyện mô hình -> sinh models/model.joblib + config.json + feature_ranges.json
...
# Chỉ cài production dependencies (loại bỏ pytest, ucimlrepo...)
COPY requirements.txt .
RUN python -m pip install --upgrade pip \
...
# CMD mặc định: chạy API service. Có thể override để chạy dashboard.
